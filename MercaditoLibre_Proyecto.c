...
//inicializarMercaditoLibre: Como bien dice su nombre, inicializa la estructura principal dandole memoria e inicializando las listas que contiene en NULL al igual que las bodegas;
...
// valida que la opcion ingresada por el usuario sea valida, de lo contrario se le pedira que la vuelva a ingresar
...
//crearNodoProveedores: crea un nodo de la misma estructura de la lista en el cual sera agregado
...
//agrega un nodo con la info del proveedor a la lista de proveedores
...
//buscarBodega: busca la bodega segun por su id en el arreglo de bodegas, una vez encontrada la retorna, caso contrario retorna NULL
...
//buscarPosicionBodega: Busca la posicion segun el id de la bodega en el arreglo de bodegas y la retorna una vez encontrada
...
//crearInfoBodegas: Crea un nodo para la bodega con la informacion recibida por consola
...
//opcionAgregar: Esta es la opcion 1 del menu, en donde se llaman las funciones necesarias para agregar tanto una bodega como un proveedor
...
//imprimirTodosProveedores: Imprime todos los proveedores que se encuentran en la lista
...
//imprimirTodasBodegas: Imprime la informacion de todas las bodegas del arreglo
...
//imprimirDetalleVenta: Imprime el detalle de los productos vendidos en una venta en especifica
...
//buscarBodegaDisponible: busca una bodega que tenga disponible el almacenamiento solicitado, de ser asi retiorna la posicion de la bodega en el arreglo
...
//buscarProductoEnABB: busca un producto por su id en el arbol y retorna sus datoas en caso de ser encontrado
...
//buscarProducto: busca el producto por su id en los arboles de cada bodega llamando a buscarProductoEnABB, si lo encuentra retorna sus datos
...
//buscarVenta: busca una venta por su id en la lista de ventas, si la encuentra retorna sus datos
...
//mostrarProveedor: muestra un proveeedor por su id o imprime todos los proveedores de la lista. Opcion del usuario
void mostrarProveedor(struct nodoProveedores *lista, int opcion){
    ...
    if(opcion == 1){
        do{
            ...
            if(datosProveedor) {
                printf("ID: %i  | NOMBRE: %s\n", datosProveedor->idProveedor, datosProveedor->nombre);
                puts("----------------------------------------");
                return;
...
//mostrarBodegas: muestra una bodega por su id o imprime todas las bodegas del arreglo. Opcion del usuario
...
//mostrarProducto: muestra un producto por su id o imprime todos los productos que se encuentran en una bodega. opcion del usuario
...
//mostrarVenta: imprime los detalles de una venta segun su id junto con los detalles de los productos comprados
...
//mostrarCompra: muestra todos los productos comprados a un proveedor
...
void mostrarRotacionProducto(struct MercaditoLibre *ML){
    ...
    if (mayorOcurrencia != -1){
        ...
        printf("ID: %i  |  NOMBRE: %s |  OCURRENCIA: %i\n\n", productoMayorRotacion.idProducto, productoMayorRotacion.nombre, mayorOcurrencia);
        printf("PRODUCTO CON MENOR ROTACION:\n");
        printf("ID: %i  |  NOMBRE: %s |  OCURRENCIA: %i\n\n", productoMenorRotacion.idProducto, productoMenorRotacion.nombre, menorOcurrencia);
        puts("----------------------------------------");
    }
    else puts("\nERROR: No existen registros de ventas.");
}

int recorrerDetalle(struct nodoVendido *lista, int id, int *cont){

    while (lista != NULL){
        if (lista->detalleProducto.idProducto == id)
            *cont += lista->detalleProducto.cantidad;
        lista = lista->sig;
    }
    return *cont;
}

int determinarVentas(struct nodoVentas *lista, int id){

    int cont = 0;
    while (lista != NULL){
        recorrerDetalle(lista->datosVenta->listaVendido, id, &cont);
        lista = lista->sig;
    }
    return cont;
}

void determinarProducto(struct nodoProductos *ABB, struct nodoVentas *lista, struct producto *productoMayor, int *ventasMayor){

    int ventas;
    if (ABB){
        determinarProducto(ABB->izq, lista, productoMayor, ventasMayor);
        ventas = determinarVentas(lista, ABB->datosProducto->idProducto);
        if (ventas > (*ventasMayor)){
            *productoMayor = *(ABB->datosProducto);
            *ventasMayor = ventas;
        }
        determinarProducto(ABB->der, lista, productoMayor, ventasMayor);
    }
}

void mostrarProductoMasVendido(struct MercaditoLibre *ML){

    struct producto productoMayorVenta;
    int ventasMayor = 0, i;
    for (i = 0 ; i < maxBodegas ; i++){
        if (ML->Bodega[i]){
            determinarProducto(ML->Bodega[i]->ABP, ML->listaVentas, &productoMayorVenta, &ventasMayor);
        }
    }
    if (ventasMayor != 0){
        printf("ID: %i  |  NOMBRE: %s |  UNIDADES VENDIDAS: %i  |  PESO (kg): %.2f |  PRECIO: %i\n\n", productoMayorVenta.idProducto, productoMayorVenta.nombre, ventasMayor, productoMayorVenta.peso, productoMayorVenta.precio);
        puts("----------------------------------------");
    }
    else puts("\nERROR: No existen registros de ventas.");
}
